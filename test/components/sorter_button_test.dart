...
void main() {
  testWidgets('SorterButton muestra opciones y ordena tareas alfabeticamente', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('SorterButton muestra opciones y ordena tareas por fecha', (
    WidgetTester tester,
  ) async {
    final taskNotifier = TaskNotifier();
    // Carga tareas de prueba
    ...
    // Selecciona "Ordenar por fecha"
    await tester.tap(find.text('Ordenar por fecha'));
    await tester.pumpAndSettle();

    // Verifica que la primera tarea ahora sea la que empieza con 'B'
    expect(taskNotifier.tasks.first.title, 'B tarea');
  });
}
